...
def action_download(doc_list, docs):
    ...
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 6.0; pl; rv:1.9.1.2) Gecko/20090729 Firefox/3.5.2 GTB7.1 ( .NET CLR 3.5.30729',
    ...
    for doc in doc_list:
        ...
        try:
            ...
            info('Error: File Not Found on the Server Side: ConnectionError')
            pass
        except requests.exceptions.HTTPError:
            info('Error: File Not Found On The Server Side: HTTPError')
            pass
        except KeyError:
            pass
    ...
    print '/tData Doenloaded: {}MB'.format(str(math.floor(data_size)))
    info(str(doc).replace('%20', ' '))
    pass

def doc_search(doamin, USERAGENT_F, prox):
    q1 = Queue.Queue()
    q2 = Queue.Queue()
    t1 = threading.Thread(target=doc_bing, args=(domain, USERAGENT_F, proxy, q1))
...
def pdf_read(pdf_file_list):
    ...
    software_list = []
    user_names = []
    for filename in pdf_file_lists:
        info(filename)
        try:
            ...
            if person: 
                ...
                if oddity:
                    ...
                    user_names.append(str(oddity).tittle())
                else:
                    user_names.append(str(person).tittle())
            if software:
                oddity2 = re.match('(\s\w\+(\w\s\+)', software)
                if oddity2:
...
def ms_doc(ms_file_list):
    ...
    for filename in ms_file_list:
        try:
            ...
            save_by = re.sub('[^0-9a-zA-Z]+', ' ', meta_last_saved_by)
            if author:
                ...
                if oddity:
                    ...
                    user_names.append(str(author).tittle())
            if software: 
                oddity2 = re.match('(\s\w\s+(\w\s+)', software)
                if oddity2:
                    oddity2 = str(oddity2.group(1)).replace(' ', ' ')
                    software_list.append(oddity2)
                else:
            ...
            if save_by:
                ...
                if oddity3:
                    ...
                    user_names.append(str(oddity3).tittle())
                else:
...
def doc_start(domain, USERAGENT_F, prox_q):
    ...
    downlaod_count = len(download_list)


    for root, dirs, files in os.walk(docs)
    for filename in files:
        if str(filename.enswith(ms_list_ext):
            ms_file_list.append(os.path.join(root, filename))
        if str(filename).endswith('.pdf'):
            psd_file_list.append(os.apth.join(root, filename))
    ...
    if user_names_t and software_list_t:
        ...
        info('The Hunt Ended')
        q.put((user_names, software_list, doenload_count, download_list))
        ...
        user_names = []
        info('The Hunt Ended')
        q.put((user_names, software_list, downlaod_count, download_list))
        ...
        software_list = []
        info('The Hunt Ended')
        q.put((user_names, software_list, download_list, downlaod_count))
